...
/**
 * Слушатель события бана через Litebans
...
public class BanEventListener implements Listener {
    ...
    /**
     * Создает новый слушатель событий бана
     * @param plugin главный класс плагина
     * @param litebansManager менеджер Litebans
     */
    public BanEventListener(Main plugin, LitebansManager litebansManager) {
        this.plugin = plugin;
    ...
    private void registerLitebansListener() {
        plugin.getLogger().info("Регистрация слушателя событий LiteBans для банов...");
        
        // Добавляем обработчик событий бана в LiteBans API
        Events.get().register(new Events.Listener() {
            @Override
            public void entryAdded(Entry entry) {
                // Обрабатываем только события бана
                if (entry.getType().equals("ban")) {
                    ...
                    // Вместо асинхронной задачи Bukkit используем обычный Java Thread
                    // Это безопасно, так как Bukkit может быть отключен в момент вызова события LiteBans
                    new Thread(() -> {
                        try {
                            litebansManager.handleBan(executorName, playerName, reason, duration);
                            plugin.getLogger().info("Обработан бан игрока " + playerName + " администратором " + executorName);
                        } catch (Exception e) {
                            plugin.getLogger().warning("Ошибка при обработке бана: " + e.getMessage());
                            e.printStackTrace();
                        }
                    }).start();
                }
    ...
    private String getPlayerNameFromEntry(Entry entry) {
        ...
        try {
            ...
            plugin.getLogger().warning("Некорректный UUID игрока в событии бана: " + uuidString);
        }
