    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {!iconOnly && <span className="mr-1">{value}</span>}
            {copied ? <Check className="h-3.5 w-3.5 text-green-500" /> : <Copy className="h-3.5 w-3.5 opacity-70" />}
            <span className="sr-only">Копировать</span>
          </Button>
        </TooltipTrigger>
        <TooltipContent side="bottom">{copied ? "Скопировано!" : "Копировать IP"}</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  )
}
...
import { Users, Server, Gift, ChevronRight, Gamepad2, Download } from "lucide-react"
          ...
          <div className="container flex h-16 items-center space-x-4 sm:justify-between sm:space-x-0">
            <div className="flex gap-6 md:gap-10">
              <Link href="/" className="flex items-center">
                <span className="inline-block text-xl font-bold bg-gradient-to-r from-[#EC39D9] to-[#FB0D68] text-transparent bg-clip-text">
                  EternalCore
                </span>
              </Link>
            ...
            <div className="flex flex-1 items-center justify-end space-x-4">
              <nav className="flex items-center space-x-2">
                <div className="hidden md:flex items-center mr-2 rounded-full bg-muted px-3 py-1.5">
                  <span className="text-sm font-medium mr-1">IP:</span>
                  <CopyButton value={serverIP} className="h-auto p-0 text-sm" />
                </div>
                <ModeToggle />
                <Button
                  variant="default"
                  className="bg-gradient-to-r from-[#EC39D9] to-[#FB0D68] text-white font-medium"
                >
              ...
              <div className="grid gap-6 lg:grid-cols-[1fr_400px] lg:gap-12 xl:grid-cols-[1fr_600px]">
                <div className="flex flex-col justify-center space-y-4">
                  ...
                  <div className="flex flex-col gap-2 min-[400px]:flex-row">
                    <Button className="bg-gradient-to-r from-[#EC39D9] to-[#FB0D68] text-white font-medium">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                    <div className="flex items-center rounded-full bg-background/80 border border-border px-3 py-1">
                    ...
                    <div className="flex items-center rounded-full bg-background/80 border border-border px-3 py-1 cursor-pointer group">
                      <span className="mr-1">IP:</span>
                      <CopyButton value={serverIP} className="h-auto p-0 text-sm" />
                    </div>
                    <div className="flex items-center rounded-full bg-background/80 border border-border px-3 py-1">
                      <span className="flex items-center">
                        <span className="mr-1.5 h-4 w-4 rounded-full bg-gradient-to-r from-[#EC39D9] to-[#FB0D68]"></span>
                        Minecraft 1.16.5
                      </span>
                    </div>
                  </div>
            ...
            <div className="container px-4 md:px-6">
              <div className="flex flex-col items-center justify-center space-y-4 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-6">
                  <Button className="bg-gradient-to-r from-[#EC39D9] to-[#FB0D68] text-white font-medium px-8 py-6 text-lg">
        ...
        <footer className="w-full border-t py-6 bg-background">
          <div className="container flex flex-col items-center justify-center gap-4 md:flex-row md:gap-8">
            <p className="text-center text-sm leading-loose text-muted-foreground md:text-left">
              © 2024 EternalCore. Все права защищены.
            </p>
